    ...
    return {
        detect(text) {
            try {
                ...
                console.error('PatternEngine.detect error:', e);
                return [];
            }
        },
        ...
        getSuggestedTransforms(pattern) {
            const map = {
                'repeat': ['shift_alphabet'],                'palindrome': ['reverse_segment'],
    ...
    return {
        apply(text, key, alphabet, transformName, pattern) {
            try {
                ...
                console.error('TransformEngine.apply error:', e);
                return { text, key };
            }        },
...
// Precompile known fragmentsfunction compileKnownFragments() {
    KNOWN_FRAGMENTS = [];
    KNOWN_REGEX = null;
...
function scoreText(text) {
    ...
    let score = 0;
    const upperText = text.toUpperCase();    const textLen = upperText.length;
