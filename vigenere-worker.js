...
const PatternEngine = (() => {
    ...
    return {
        detect(text) {
            ...
            const vRatio = vCount / Math.min(SCAN_LIMIT, text.length);
            if (vRatio > 0.55) {                patterns.push({ type: 'vc_high', ratio: vRatio });
            } else if (vRatio < 0.25) {
            ...
            if (KNOWN_REGEX && KNOWN_FRAGMENTS.length > 0) {
                for (const frag of KNOWN_FRAGMENTS) {
                    ...
                    if (idx !== -1) {
                        ...
                        if (context.length > 0 && noise / context.length > 0.3) {
...
onmessage = function(e) {
    if (e.data.type === 'init') {        ALPHABET = e.data.alphabet || "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
